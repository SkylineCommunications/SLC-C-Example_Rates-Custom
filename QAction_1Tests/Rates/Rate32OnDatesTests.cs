...
{
	[TestClass()]
	public class Rate32OnDatesTests
	{
		...
		public void CalculateInvalid_BackInTime()
		{
			...
			var helper = Rate32OnDates.FromJsonString("", minDelta, maxDelta);
		...
		public void Serialize_Invalid_DifferentCounter()
		{
			// Arrange
			DateTime start = new DateTime(2000, 1, 1);
			var helper1 = Rate32OnDates.FromJsonString("", minDelta, maxDelta);
			helper1.Calculate(5, start);
			helper1.Calculate(10, start.AddSeconds(10));

			string serializedTemp = helper1.ToJsonString();
			var helper2 = Rate32OnDates.FromJsonString(serializedTemp, minDelta, maxDelta);
		...
		private static void AddSameToBoth(Rate32OnDates helper1, Rate32OnDates helper2, uint newCounter, DateTime time)
		{
